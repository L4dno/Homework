...
struct Cat {
	double attack_dist = 0;
	Rdec2D direction = { 0,0 };
	double speed = 0;
	Rdec2D pos = { 0,0 };
};
...
bool EscapeCondition(const Mouse& m){
	if (m.pos.y >= 0) {
		return true;
	}
	else {
...
bool IsCaught(const Cat& c, const Mouse& m) {
	if (Norm(m.pos - c.pos) <= c.attack_dist) {
		return true;
	}
	else {
		return false;
...
void UpdateCat(Cat& c, const Mouse& m) {
	auto tmp = c.direction + m.direction;
	c.direction = tmp / Norm(tmp);
}
